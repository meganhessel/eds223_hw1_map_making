...
### Trying to download USA State polygons 

```{r}

tmap_arrange(peopcolor, sfp)


tm_shape(EBRP_LA) + 
  tm_polygons(fill = 'P_D2_RSEI_AIR') # Percentile for Toxic Releases to Air EJ Index


tm_shape(EBRP_LA) + 
  tm_polygons(fill = 'P_CANCER') # % cancer


low_income <- tm_shape(EBRP_LA) + 
  tm_polygons(fill = 'P_LOWINCPCT') #% low income 


under_st <- tm_shape(EBRP_LA) + 
  tm_polygons(fill = 'P_D5_UST') # % Underground storage tanks Supplemental Index

tmap_arrange(peopcolor, low_income, sfp, under_st)




tm_shape(us_states$geometry) + 
  tm_polygons()
...
# Download state polygons for the US as an sf object
